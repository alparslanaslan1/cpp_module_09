...
void BitcoinExchange::inputData(std::string inputFile) {
    ...
    while (std::getline(inputDataFile, line)) {
        ...
        if (i == std::string::npos) {
            date = BitcoinExchange::inputDateValue(datePart);
            amount = std::numeric_limits<float>::quiet_NaN();
        }
        else
        {    
            ...
            date = BitcoinExchange::inputDateValue(datePart);
            amount = BitcoinExchange::inputPriceValue(pricePart);
        }
    ...
    for (itInput = inputBase.begin(); itInput != inputBase.end(); ++itInput) {
        float closestValue = getClosestValue(itInput->first);
